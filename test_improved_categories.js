...
const CATEGORIES = {
  ...
  "Cloud & Hosting": [
    "cloud", "hosting", "server", "aws", "azure", "digitalocean", "linode", "vultr",
    "heroku", "netlify", "vercel", "cloudflare", "firebase", "supabase", "planetscale",
    "railway", "render", "fly.io", "deno", "anaconda", "genesis", "citadel", "tebex"
  ],
  "Domain & DNS": [
    "domain", "dns", "whois", "registrar", "nameserver", "dnsimple", "dyn", "epik",
    "freedns", "nextdns", "cloudns", "route53", "godaddy", "namecheap", "hover"
  ],
  "File Storage & Sharing": [
    "file", "upload", "download", "share", "storage", "drive", "sync", "backup",
    "transfer", "1fichier", "4shared", "mediafire", "mega", "sendspace", "wetransfer",
    "dropbox", "box", "onedrive", "icloud", "pcloud", "storj", "alldebrid"
  ],
  "Anime & Manga": [
    "anime", "manga", "anilist", "anidb", "myanimelist", "crunchyroll", "funimation",
    "mangadex", "animeplanet", "kitsu", "mal", "otaku", "weeb", "animoto"
  ],
  "Forums & Communities": [
    "forum", "forums", "community", "board", "discussion", "talk", "reddit", "discourse",
    "phpbb", "vbulletin", "xenforo", "invision", "flarum", "vanilla", "nodebb"
  ],
  "Fashion & Lifestyle": [
    "fashion", "style", "clothing", "shoes", "accessories", "lifestyle", "beauty",
    "zalando", "asos", "h&m", "zara", "uniqlo", "nike", "adidas", "aldo", "vinted",
    "laredoute", "decathlon", "boutique"
  ],
  "Q&A & Knowledge": [
    "quora", "ask", "answer", "question", "knowledge", "wiki", "faq", "help", "support",
    "stackoverflow", "stackexchange", "yahoo answers", "answers", "brainly", "chegg"
  ],
  "Maps & Navigation": [
    "maps", "navigation", "gps", "directions", "location", "openstreetmap", "mapstr",
    "waze", "tomtom", "garmin", "here", "mapbox", "foursquare", "yelp"
  ],
  "Cryptocurrency & Blockchain": [
    "crypto", "cryptocurrency", "blockchain", "bitcoin", "ethereum", "defi", "nft",
    "coinbase", "binance", "kraken", "metamask", "uniswap", "opensea", "braiins"
  ],
  Other: []
};
...
const otherCount = categoryStats['Other'] || 0;
const categorizedCount = data.length - otherCount;
const coverageRate = ((categorizedCount / data.length) * 100).toFixed(1);

console.log('');
console.log(`分类覆盖率: ${coverageRate}%`);
console.log(`未分类网站: ${otherCount} (${((otherCount / data.length) * 100).toFixed(1)}%)`);

// 检查WeChat的分类
const wechatGuide = data.find(guide => 
  guide.name.toLowerCase().includes('wechat') || 
  guide.domains.some(domain => domain.includes('wechat'))
);

if (wechatGuide) {
  const wechatCategory = categorizeGuide(wechatGuide);
  console.log('');
  console.log(`WeChat分类结果: ${wechatCategory}`);
  console.log(`WeChat信息: ${wechatGuide.name} - ${wechatGuide.domains.join(', ')}`);
}

console.log('');
console.log('=== 前30个未分类网站 ===');
otherSites.slice(0, 30).forEach((site, index) => {

...
export function CategoryPageClient({ guides: initialGuides, categoryName }: CategoryPageClientProps) {
  const [currentPage, setCurrentPage] = useState(1)
  const { currentLanguage } = useLanguage()
  const [guides, setGuides] = useState<ProcessedGuide[]>(initialGuides)
  const t = useTranslations()

  // 分类名称转翻译键（与导航和分类列表页保持一致）
  ...
  // 当语言改变时重新获取数据
  useEffect(() => {
    const localizedGuides = getGuidesByCategory(categoryName, currentLanguage)
